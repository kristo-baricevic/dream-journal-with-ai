...
const parser = StructuredOutputParser.fromZodSchema(
    z.object({
        ...
        color: z
          .string()
          .describe(
            'a hexidecimal color code that represents the mood of the entry. Decide between red for angry, yellow for happy, green for weird, blue for sad, purple for exciting or romantic. If an entry does not match these, make it up. Just avoid white, gray, and black.'
          ),
        interpretation: z
          .string()
          .describe(
...
const generateDreamPrompt = async () => {
  const format_instructions = parser.getFormatInstructions()

  const dreamPrompt = new PromptTemplate({
    template:
      'You are doctor of dream analysis. You have a Phd in Psychology with a Masters in Dream Analysis from the University of Dreams. You have magical powers to interpret dreams. Analyze the following journal entry. Follow the instructions and format your response to match the format instructions, no matter what! When you are done, suggest a song to listen to and a snack to eat.\n{format_instructions}\n{entry}',
    inputVariables: ['entry'],
    partialVariables: { format_instructions },
  ...
  const journalEntry = "This is a sample dream journal entry. It's about a dream I had last night...";
  const dreamInput = await dreamPrompt.format({ entry: journalEntry });
  
  return dreamInput;
};

export const generateDream = async () => {
  const input = await generateDreamPrompt();
  const model = new OpenAI({temperature: 0, modelName: 'gpt-3.5-turbo'});
  const result = await model.call(input);
